<template>
    <n-card :class="`shadow-md`" size="small" :segmented="{
        ...
        <template #header>Form Tarik Rekening Koran</template>
...
<span class="hidden md:flex">pindah ke penerimaan</span>
  ...
  >
    ...
    :on-update:checked-row-keys="handleFasilitas" :loading="loadSearch" class="pb-2" v-show="dataFasilitas" /> -->
        <n-data-table :row-props="rowProps" striped :row-class-name="rowClassName" size="small"
        ...
        <n-spin v-if="displayDetail" :show="spinnerShow">
            ...
            <div>
                <n-table size="small" v-show="props.embed ? true : displayFasilitas" class="mb-2" single-column
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td align="right">{{ formatter.format(pelunasan.NILAI_PINJAMAN) }}</td>

                        </tr>
                        <tr>
                            <td>Nilai Pembayaran</td>
                            <td align="right">{{ formatter.format(pelunasan.NILAI_PEMBAYARAN) }}</td>
                ...
                <n-divider title-placement="left">Topup</n-divider>
                <div class="md:flex gap-2 bg-slate-50 shadow  border rounded-xl items-center pt-4 px-4"
                    v-show="props.embed ? true : displayFasilitas">

                    <n-form-item path="nestedValue.path2" label="Nominal Topup" class="w-full">
                        <n-input-number v-bind:dir="isRtl ? 'rtl' : 'ltr'" placeholder="Jumlah Pembayaran"
                            v-model:value="pelunasan.NOMINAL" :show-button="false" :parse="parse" :format="format"
                            clearable @clear="clearUangPelaanggan" @blur="pushJumlahUang" class="w-full"
                            ref="uangPelangganRef" @click="handleUangPelangganFocus" :max="pelunasan.NILAI_PEMBAYARAN">
                        </n-input-number>
                    </n-form-item>
                    <n-button @click="() => (pelunasan.NOMINAL = pelunasan.NILAI_PEMBAYARAN / 100 * 10)">10%</n-button>
                    <n-button @click="() => (pelunasan.NOMINAL = pelunasan.NILAI_PEMBAYARAN / 100 * 50)">50%</n-button>
                    <n-button
                        @click="() => (pelunasan.NOMINAL = pelunasan.NILAI_PEMBAYARAN / 100 * 100)">100%</n-button>
                    <n-form-item label="Keterangan" class="w-full">
                        <n-input type="textarea" autosize :show-button="false" class="w-full"
                            v-model="pelunasan.KETERANGAN" />
                    </n-form-item>
...
const handleSearch = async () => {
    ...
    const response = await useApi({
        method: "POST",
        api: "search_customer_rekening_koran",
        data: dynamicSearch,
        token: userToken,
    });
    if (!response.ok) {
