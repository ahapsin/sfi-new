<template>
  <div class="flex flex-col gap-1">
    <div v-for="menu in props.menus" :key="menu.menuid">
      ...
      <div
        class="flex items-center justify-between gap-2 text-pr cursor-pointer"
        @click="toggleMenu(menu.menuid)"
      >
        <div class="flex items-center gap-2 w-full p-2 rounded-lg hover:bg-pr-50">
      ...
      <transition name="fade">
        <div
          v-if="openMenus.includes(menu.menuid)"
          class="ms-4 border-l border-pr-200 ps-2 mt-1"
        >
          <div
            v-for="submenu in [...menu.menuitem.submenu].sort(
              (a, b) => a.sublabel.localeCompare(b.sublabel)
            )"
            :key="submenu.subroute"
          >
            <router-link
              :to="`${menu.menuitem.routename}${submenu.subroute}`"
              v-slot="{ isActive }"
              @click="handleStart"
            >
              <div
                class="flex transition-all gap-2 rounded-xl p-2 w-full cursor-pointer"
                :class="
                  isActive
